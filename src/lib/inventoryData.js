...
export async function loadProducts(filters = {}) {
  ...
  const data = await api.get(`/inventory/productos/${query}`);
  products = Array.isArray(data) ? data.map(mapProductFromApi) : [];
  return getProducts();
}

export async function loadUbicaciones() {
  const data = await api.get('/inventory/ubicaciones/');
  ubicaciones = Array.isArray(data) ? data : [];
  return ubicaciones;
}

export async function getCategorias() {
  const data = await api.get('/inventory/productos/categorias/');
...
export async function addProduct(product) {
  ...
  const created = await api.post('/inventory/productos/', payload);
...
export async function updateProduct(id, updatedData) {
  ...
  const updated = await api.put(`/inventory/productos/${id}/`, payload);
...
export async function deleteProduct(id) {
  await api.delete(`/inventory/productos/${id}/`);
...
export async function recordMovement(movement) {
  ...
  const created = await api.post('/inventory/movimientos/', payload);
...
export async function getInventoryMovements(filters = {}) {
  ...
  const query = params.toString() ? `?${params.toString()}` : '';
  const data = await api.get(`/inventory/movimientos/${query}`);
